...
class DwMaterialApp extends ConsumerWidget {
  const DwMaterialApp({
    ...
    required this.flutterAppOptionsProvider,
    required this.title,
    this.appWrapper,
  });

  final ProviderBase<RouterConfig<Object>> routerProvider;
  final ProviderBase<DwFlutterAppOptions> flutterAppOptionsProvider;
  final String title;
  final Function(BuildContext context, Widget child)? appWrapper;
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final router = ref.watch(routerProvider);
    final flutterAppOptions = ref.watch(flutterAppOptionsProvider);
    ...
    return MaterialApp.router(
      ...
      supportedLocales: dw.supportedLocales,
